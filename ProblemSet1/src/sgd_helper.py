...
def plot_loss_function(X_grid, Y_grid, loss_grid):
    ...
    surf = ax.plot_surface(X_grid, Y_grid, loss_grid, color='yellow', 
                        alpha = 1, zorder=0)
...
def animate_sgd_suite(SGD, loss, X, Y, params, N_epochs, step, ms=1):
    ...
    # Plot w_start values.
    _, = ax.plot(W_lst[:, 0, 0], W_lst[:, 0, 1], losses_lst[:, 0], '+', 
                    mew=2, ms=10, c='black')
    
    # Initialize graph to animate on.
    graph, = ax.plot([], [], [], 'o', ms=0.5, c='black')
    ...
    def animate(i):
        if i > delay:
            ...
            graph.set_data(W_lst[:, :step*(i+1), 0].flatten(), \
                                    W_lst[:, :step*(i+1), 1].flatten())
    ...
    # Animate!
    print('\nAnimating...')
    anim = FuncAnimation(fig, animate, frames=int(N_epochs/step)+delay, 
                            interval=50)
...
def animate_convergence(X, Y, W, step):
    ...
    # Animate!
    print('\nAnimating...')
    anim = FuncAnimation(fig, animate, frames=int(len(W)/step)+delay, 
                        interval=50)
