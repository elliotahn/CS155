...
def generate_dataset(N_train, N_test, n_spirals, r):
    ...
    # The second half of the points will be rotated by pi to create the 
    # second spiral.
...
def visualize_predictions(X, Y, Y_pred, title, fname):
    # Set colormap such that red is positive and blue is negative.
    plt.set_cmap('bwr')
    plt.figure(figsize=(6, 5))

    # Plot data.
    plt.scatter(X[:, 0], X[:, 1], c=Y, s=10, linewidth=np.abs(Y - Y_pred), 
                        edgecolors='black')
    plt.title(title)
    plt.colorbar()
...
def visualize_loss_curves_gb(model, X_train, Y_train, X_test, Y_test):
    ...
    for clf in model.clfs:
        ...
        losses_train.append(len(np.where(np.sign(Y_train_pred) != \
                        Y_train)[0]) / len(Y_train_pred))
        losses_test.append(len(np.where(np.sign(Y_test_pred) != Y_test)[0]) \
                        / len(Y_test_pred))
...
def gb_suite(Boosting, n_clfs, X_train, Y_train, X_test, Y_test, fname):
    # Fit a boosting model.
    model = Boosting(n_clfs=n_clfs)
    model.fit(X_train, Y_train)

    # Report the prediction plots and losses.
    visualize_predictions(X_train, Y_train, model.predict(X_train), 
                    'Training dataset predictions', fname + '_train.png')
    print('Training loss: %f' % model.loss(X_train, Y_train))

    visualize_predictions(X_test, Y_test, model.predict(X_test), 
                    'Test dataset predictions', fname + '_test.png')
    print('Test loss: %f' % model.loss(X_test, Y_test))

    return model

# Same as above but also returns D (dataset weights across all iterations).
def ab_suite(AdaBoost, n_clfs, X_train, Y_train, X_test, Y_test, fname):
    # Fit a boosting model.
...
def animate_gb(model, X, Y, title):
    ...
    anim = animation.FuncAnimation(fig, animate, 
        frames=np.arange(int(model.n_clfs / step)), fargs=(Y_clfs, scat))
